...
const fieldClass =
  'w-full px-4 py-3 rounded-xl border border-surface-variant bg-surface-container-low text-on-surface font-body-base text-body-base focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all duration-160'
...
const ContactSettings = () => {
  ...
  useEffect(() => {
    api
      ...
      .catch(() => console.warn('ContactSettings: failed to load contact'))
  ...
  return (
    <div>
      ...
      <form
        ...
        className="bg-surface-container-lowest rounded-xl shadow-ambient p-6 max-w-2xl animate-fade-up"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
          ...
          <div>
            ...
            <input
              value={form.hours_ar}
              onChange={(e) => setForm((f) => ({ ...f, hours_ar: e.target.value }))}
              className={fieldClass}
            />
          </div>
          <div>
          ...
          <div>
            <label className="block font-body-sm text-body-sm text-on-surface mb-1">
              استعلام خرائط (maps_query)
            </label>
            <input
              value={form.maps_query}
              onChange={(e) => setForm((f) => ({ ...f, maps_query: e.target.value }))}
              className={fieldClass}
            />
          </div>
        </div>
        ...
        {message && (
          <div
            className={`mb-4 px-4 py-3 rounded-xl text-body-sm animate-fade-in ${
              messageType === 'success'
