...
const CategoriesManager = () => {
  ...
  const fetchData = () => {
    api
      .getCategories()
      .then(setCategories)
      .catch(() => console.warn('CategoriesManager: failed to load'))
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((cat, i) => (
            <div
              key={cat.id}
              className="bg-surface-container-lowest rounded-xl shadow-ambient p-4 transition-transform duration-200 ease-out-strong hover:-translate-y-1 hover:shadow-ambient-hover animate-scale-in"
            ...
            >
              {cat.image_url && (
                <img
                  src={cat.image_url}
                  alt={cat.name_ar}
                  className="w-full h-32 object-cover rounded-lg mb-3 transition-transform duration-160 ease-out-strong hover:scale-[1.02]"
                  onError={(e) => {
                    e.target.style.display = 'none'
                  }}
                />
